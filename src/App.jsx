import React from 'react'
import { Contact, Home, Navbar, Projects, About } from './components'
import { githubWhite, instagramWhite, linkedinWhite} from './assets'

const App = () => {
  return (
    <>
      <div className="flex flex-col h-full w-full font-sans ">
        <Navbar/>
      </div>
      <section id="home">
        <Home/>
      </section>
      <section id="about">
        <About/>
      </section>
      <section id="projects">
        <Projects/>
      </section>
      <section id="contact">
        <Contact/>
      </section>
      <div className="flex sm:flex-row flex-col justify-between w-full bg-[#2d2e32] text-white sm:px-52 p-10 gap-5">
        <p className="flex sm:text-xl text-base justify-center font-bold">Copyright © 2024. All rights are reserved</p>
        <div className="flex flex-row gap-5 items-center justify-center sm:justify-normal">
            <a href=""><img className='flex' src={linkedinWhite} alt="LinkedInWlinkedinWhite"/></a>
            <a href=""><img className='flex' src={githubWhite} alt="GitHubWhigithubWhite"/></a>
            <a href=""><img className='flex' src={instagramWhite} alt="InstagramWhiteinstagramWhite"/></a>
        </div>
      </div>
    </>
  )
}

export default App